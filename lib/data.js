...
// Soal
export const getSoalByIdSoal = async (id) => {
  try {
    const response = await axios.get(
      `http://localhost:5000/api/latihan/detail/${id}/soal`
    );
    ...
    console.error("Error fetching data soal by id soal:", error);
    throw new Error("Gagal mengambil data soal by id soal.");
  }
};

export const getSoalByLatihanId = async (latihanId) => {
  try {
    const response = await axios.get(
      `http://localhost:5000/api/latihan/${latihanId}/soal`
    );
    ...
    console.error("Error fetching data soal by id latihan:", error);
    throw new Error("Gagal mengambil data soal by id latihan.");
  }
};
...
export const updatedSoal = async (id, updatedDataSoal) => {
  try {
    const response = await axios.patch(
      `http://localhost:5000/api/latihan/detail/:id/soal/${id}`,
      updatedDataSoal
    );
    if (!response.data.success || !Array.isArray(response.data.data)) {
      throw new Error("Data tidak valid");
