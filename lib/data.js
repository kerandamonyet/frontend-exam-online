...
export const getSoalByLatihanId = async (latihanId, page = 1, limit = 10) => {
  try {
    const response = await axios.get(
      `http://localhost:5000/api/soal/${latihanId}/soal?page=${page}&limit=${limit}`
    );
...
export async function getSesiLatihan(id) {
  try {
    const response = await axios.get(
    ...
    console.error("Error fetching data sesi latihan by id:", error);
    throw new Error("Gagal mengambil data sesi latihan berdasarkan id.");
  }
}
