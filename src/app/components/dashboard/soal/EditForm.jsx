...
function EditModal({ isOpen, onClose, onSubmit, siswa }) {
  ...
  // Muat data awal (jika ada) dari prop "siswa"
  useEffect(() => {
    if (siswa) {
      setFormData({
        kelas: siswa.kelas || "",
        tipe_latihan: siswa.tipe_latihan || "AKM",
      });
      // Pastikan siswa.tgl_mulai dan siswa.tgl_selesai merupakan string tanggal yang valid
      setStartDate(new Date(siswa.tgl_mulai));
      setEndDate(new Date(siswa.tgl_selesai));
    }
  }, [siswa]);
  ...
  // Handle perubahan input teks dan number
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Misalnya, endpoint mengharuskan penggunaan id siswa/latihan
      await axios.patch(`http://localhost:5000/api/latihan/${siswa.id}`, {
